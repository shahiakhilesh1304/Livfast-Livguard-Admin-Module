...
public class LivFastTertiaryCSVHelper 
{
	...
	public static List<LivFastTertiaryScheme> csvToTutorials(InputStream is, int schemeID) 
	{
		try (BufferedReader fileReader = new BufferedReader(new InputStreamReader(is, "UTF-8"));
				CSVParser csvParser = new CSVParser(fileReader, CSVFormat.DEFAULT.withFirstRecordAsHeader().withIgnoreHeaderCase().withTrim());) 
		{
			List<LivFastTertiaryScheme> listTertiary = new ArrayList<LivFastTertiaryScheme>();
			Iterable<CSVRecord> csvRecords = csvParser.getRecords();
			for (CSVRecord csvRecord : csvRecords) 
			{
			
				LivFastTertiaryScheme tertiaryScheme = new LivFastTertiaryScheme("LIVGUARD", new Date(), new Date(), csvRecord.get("segment"), 
