...
public class LivFastSecondaryCSVHelper 
{
	...
	public static List<LivFastSecondaryScheme> csvToTutorials(InputStream is, int schemeID) 
	{
		try (BufferedReader fileReader = new BufferedReader(new InputStreamReader(is, "UTF-8"));
				CSVParser csvParser = new CSVParser(fileReader, CSVFormat.DEFAULT.withFirstRecordAsHeader().withIgnoreHeaderCase().withTrim());) 
		{
			System.out.println("Entered Try H");
			List<LivFastSecondaryScheme> listCoupon = new ArrayList<LivFastSecondaryScheme>();
			Iterable<CSVRecord> csvRecords = csvParser.getRecords();
			for (CSVRecord csvRecord : csvRecords) 
			{
				System.out.println("In for loop h");
				LivFastSecondaryScheme secondaryScheme = new LivFastSecondaryScheme(csvRecord.get("product_name"), csvRecord.get("brand_name"), csvRecord.get("range"), "",Integer.parseInt(csvRecord.get("loyalty_points")), schemeID);
