...
import com.livfast.repository.LivFastTertiarySchemeRepository;
import com.livfast.utility.LivFastTertiaryCSVHelper;

@Service
@Transactional("secondaryTransactionManager")
public class LivFastTertiaryServiceScheme
{
private static final Logger log = LoggerFactory.getLogger(LivFastTertiaryServiceScheme.class); 
	
	@Autowired
	LivFastTertiarySchemeRepository tertiarySchemeRepository;
	
	public String saveTertiaryScheme(MultipartFile file, int id)
	{
		try 
		{	
			List<LivFastTertiaryScheme> tertiaryList = LivFastTertiaryCSVHelper.csvToTutorials(file.getInputStream(), id);
	...
	public String editTertiaryScheme(LivFastTertiaryScheme tertiaryScheme)
	{
		try
		{
			//TertiaryScheme ts  = this.tertiarySchemeRepository.findByBatteryModel(tertiaryScheme.getBatteryModel());
			Optional<LivFastTertiaryScheme> ts  = this.tertiarySchemeRepository.findById(tertiaryScheme.getId());
			if(ts.get() != null)
			{
	...
	public List<LivFastTertiaryScheme> getTertiarySchemeList()
	{
		try
		{
	...
	public List<LivFastTertiaryScheme> getSearchTertiary(String model)
	{
		try
		{
